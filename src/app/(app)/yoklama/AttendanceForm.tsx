...
export default function AttendanceForm({
  ...
  slot: number;
  students: Student[];
  initialRecords: { studentId: number; status: AttendanceStatus; note: string | null }[];
}) {
  const [entries, setEntries] = useState<Record<number, Entry>>(() =>
    Object.fromEntries(
      initialRecords.map((record) => [record.studentId, { status: record.status, note: record.note ?? "" }]),
    ),
  );
  ...
  function update(studentId: number, patch: Partial<Entry>) {
    ...
    setMessage(null);
    setEntries((current) => {
      const prev = current[studentId] ?? { status: null, note: "" };
      return { ...current, [studentId]: { ...prev, ...patch } };
    });
  }
  ...
  function save() {
    startTransition(async () => {
      ...
      if ("error" in result) {
        ...
        setDirty(false);
        setMessage({ type: "ok", text: `Yoklama kaydedildi. ${result.markedCount} öğrenci işaretli.` });
      }
    });
  }
              ...
              return (
                <tr key={student.id} className={entry?.status ? "bg-red-50/40" : undefined}>
                  ...
                  <td className="font-medium whitespace-nowrap">
                    <Link href={`/ogrenciler/${student.id}`} target="_blank" className="hover:text-indigo-600">
                      {student.fullName}
                    </Link>
                  </td>
                  ...
                  <td className="min-w-56">
                    {entry?.status && (
                      <input
                        ...
                        placeholder="Örn. 1. ve 2. ders, hasta..."
                        value={entry.note}
                        maxLength={255}
                        onChange={(event) => update(student.id, { note: event.target.value })}
                      />
                    )}
                  </td>
                </tr>
